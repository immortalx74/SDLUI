...
void SDLUI_Init(SDL_Renderer *r, SDL_Window *w)
{
    ...
    TTF_Init();
    SDLUI_Core.font.handle = TTF_OpenFont("liberation-mono.ttf", SDLUI_Core.font.size);
    SDLUI_Core.font.height = TTF_FontHeight(SDLUI_Core.font.handle);
    TTF_SizeText(SDLUI_Core.font.handle, "0", &SDLUI_Core.font.width, &SDLUI_Core.font.height);


    SDLUI_Core.renderer = r;
    SDLUI_Core.window = w;
    SDL_GetWindowSize(SDLUI_Core.window, &SDLUI_Core.window_width, &SDLUI_Core.window_height);
    SDLUI_Core.pool = (SDLUI_Control*)malloc(SDLUI_MAX_CONTROLS * sizeof(SDLUI_Control));
}
...
//{
//SDL_RenderDrawPoint(SDLUI_Core.renderer, offsetx - x, offsety + y);
//SDL_RenderDrawPoint(SDLUI_Core.renderer, offsetx + x, offsety + y);
//SDL_RenderDrawPoint(SDLUI_Core.renderer, offsetx - x, offsety - y);
//SDL_RenderDrawPoint(SDLUI_Core.renderer, offsetx + x, offsety + y);
//}
...
//i32 T = 0;
//
//SDL_SetRenderDrawColor(SDLUI_Core.renderer, 255, 255, 255, SDL_ALPHA_OPAQUE);
...
//{
//alpha = d;
//SDL_SetRenderDrawColor(SDLUI_Core.renderer, 255, 255, 255, alpha);
//draw_point(posx, posy, i, j);
//
//if(i != j)
//{
...
void SDLUI_Render_Button(SDLUI_Control_Button *btn)
{
    if(btn->visible)
    {
        ...
        switch (btn->state)
        {
            ...
            SDL_RenderCopy(SDLUI_Core.renderer, btn->t_back_normal, NULL, &r);
            break;

            case SDLUI_BUTTON_STATE_HOVER:
            SDL_RenderCopy(SDLUI_Core.renderer, btn->t_back_hover, NULL, &r);
            break;

            case SDLUI_BUTTON_STATE_CLICK:
            SDL_RenderCopy(SDLUI_Core.renderer, btn->t_back_click, NULL, &r);
            break;
        }
        ...
        i32 w = SDLUI_Core.font.width * strlen(btn->caption);
        i32 diffx = btn->w - w;
        i32 diffy = (btn->h - SDLUI_Core.font.height)/2 + 1;

        r = {btn->x + btn->align * diffx/2, btn->y + diffy, w, SDLUI_Core.font.height};
        SDL_RenderCopy(SDLUI_Core.renderer, btn->t_text, NULL, &r);
    }
}
...
void SDLUI_Render_Tabcontainer(SDLUI_Control_TabContainer *tbc)
{
    if(tbc->visible)
    {
        ...
        for (int i = 0; i < tbc->num_tabs; ++i)
        {
            ...
            SDL_RenderCopy(SDLUI_Core.renderer, tbc->tabs[i].t_text, NULL, &dst);
            r.x += r.w;
        }
...
void SDLUI_Render_Label(SDLUI_Control_Label *lbl)
{
    if(lbl->visible)
    {
        ...
        SDL_RenderCopy(SDLUI_Core.renderer, lbl->t_text, NULL, &r);
    }
}
...
void SDLUI_Render_Text(SDLUI_Control_Text *txt)
{
    if(txt->visible)
    {
        ...
        SDL_RenderCopy(SDLUI_Core.renderer, txt->t_text, NULL, &r);
    }
}
...
void SDLUI_Render_RadioButton(SDLUI_Control_RadioButton *rdb)
{
    if(rdb->visible)
    {
        ...
        SDL_RenderCopy(SDLUI_Core.renderer, rdb->t_img, &src, &r);
    }
}

void SDLUI_Render()
{
    for (int i = 0; i < SDLUI_Core.count; ++i)
    {
        switch (SDLUI_Core.pool[i].type)
        {
            case SDLUI_CONTROL_TYPE_BUTTON:
            {
                SDLUI_Control_Button *btn = &SDLUI_Core.pool[i].button;
            ...
            {
                SDLUI_Control_SliderInt *si = &SDLUI_Core.pool[i].slider_int;
            ...
            {
                SDLUI_Control_CheckBox *chk = &SDLUI_Core.pool[i].checkbox;
            ...
            {
                SDLUI_Control_ToggleButton *tb = &SDLUI_Core.pool[i].toggle_button;
            ...
            {
                SDLUI_Control_TabContainer *tbc = &SDLUI_Core.pool[i].tab_container;
            ...
            {
                SDLUI_Control_Label *lbl = &SDLUI_Core.pool[i].label;
            ...
            {
                SDLUI_Control_Text *txt = &SDLUI_Core.pool[i].text;
            ...
            {
                SDLUI_Control_RadioButton *rdb = &SDLUI_Core.pool[i].radio_button;
...
{
    i32 offset = (u8*)ctrl - (u8*)SDLUI_Core.pool;
...
{
    if(SDLUI_Core.count < SDLUI_MAX_CONTROLS)
    {
        SDLUI_Control *ctrl = &SDLUI_Core.pool[SDLUI_Core.count];
        SDLUI_Core.count++;
        ...
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,caption, c);
        ctrl->button.t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
        ctrl->button.t_back_normal = SDL_CreateTexture(SDLUI_Core.renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, ctrl->button.w - 2, ctrl->button.h - 2);
...
void SDLUI_CreateTab(SDLUI_Control_TabContainer *tbc, char *caption)
{
    if(tbc->num_tabs == 0)
    {
        ...
        tbc->tabs[0].index = 0;
        tbc->tabs[0].width = (tbc->tabs[0].string.length * SDLUI_Core.font.width) + (2 * SDLUI_MARGIN);
        SDL_Color c = {255, 255, 255, 255};
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,caption, c);
        tbc->tabs[0].t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
    ...
    else
    {
        ...
        tbc->tabs[tbc->num_tabs - 1].width = (tbc->tabs[tbc->num_tabs - 1].string.length * SDLUI_Core.font.width) + (2 * SDLUI_MARGIN);

        SDL_Color c = {255, 255, 255, 255};
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,caption, c);
        tbc->tabs[tbc->num_tabs - 1].t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
    }
...
SDLUI_Control_Label *SDLUI_CreateLabel(i32 x, i32 y, char *caption)
{
    if(SDLUI_Core.count < SDLUI_MAX_CONTROLS)
    {
        ...
        ctrl->label.w = len * SDLUI_Core.font.width;
        ctrl->label.h = SDLUI_Core.font.height;

        SDL_Color c = {255, 255, 255, 255};
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,caption, c);
        ctrl->label.t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
...
SDLUI_Control_Text *SDLUI_CreateText(i32 x, i32 y, char *caption)
{
    if(SDLUI_Core.count < SDLUI_MAX_CONTROLS)
    {
        ...
        ctrl->text.w = len * SDLUI_Core.font.width;
        ctrl->text.h = SDLUI_Core.font.height;

        SDL_Color c = {255, 255, 255, 255};
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,ctrl->text.string.data, c);
        ctrl->text.t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
...
SDLUI_Control_RadioButton *SDLUI_CreateRadioButton(i32 x, i32 y, i32 group)
{
    if(SDLUI_Core.count < SDLUI_MAX_CONTROLS)
    {
        ...
        ctrl->radio_button.t_img = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
...
bool SDLUI_Text(SDLUI_Control_Text *txt)
{
    ...
    if(txt->string.changed)
    {
        ...
        txt->w = len * SDLUI_Core.font.width;

        SDL_Color c = {255, 255, 255, 255};
        SDL_Surface *s = TTF_RenderText_Blended(SDLUI_Core.font.handle,txt->string.data, c);
        txt->t_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
        SDL_FreeSurface(s);
...
struct __SDLUI_Memory
{
    ...
    void sdlui_alloc()
    {
        capacity = SDLUI_MEM_BLOCK;
        start = malloc(capacity);
    }

    void sdlui_realloc(size_t num_bytes)
    {
        capacity += num_bytes;
    ...
    void ensure_capacity(size_t num_bytes)
    {
        size_t bytes_left = capacity - used;

        if(bytes_left < num_bytes)
        {
            size_t needed = num_bytes - bytes_left;
            i32 chunks = (needed / SDLUI_MEM_BLOCK) + 1;
            sdlui_realloc(chunks * SDLUI_MEM_BLOCK);
        }
    }

    char *reserve(size_t num_bytes)
    {
        ensure_capacity(num_bytes);
        char *result = (start + used);
        used += num_bytes;
        //return ((char*)start + used);
        return result;
    }
}SDLUI_Memory;
...
struct SDLUI_Allocator
{
    ...
    {
        ensure_capacity(num_bytes);
        char *result = ((char*)start + used);
        used += num_bytes;
        return result;
    }
};

SDLUI_Allocator SDLUI_ControlStorage;
SDLUI_Allocator SDLUI_ControlPointers;
