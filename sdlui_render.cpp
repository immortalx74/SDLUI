...
void SDLUI_Render_ScrollArea(SDLUI_Control_ScrollArea *sa)
{
	if(sa->visible)
	{
		...
		if(sa->content_height > sa->h)
		{
			// vertical scrollbar
			SDLUI_SetColor(SDLUI_Base.theme.col_border);
			r = {xx + sa->w - sa->scrollbar_thickness, yy, sa->scrollbar_thickness, sa->track_length_v};
			SDL_RenderFillRect(SDLUI_Base.renderer, &r);

			SDLUI_SetColor(SDLUI_Base.theme.col_thumb);
			sa->thumb_size_v = (float)(sa->track_length_v * sa->h) / (float)sa->content_height;
		...
		if(sa->content_width > sa->w)
		{
			...
			SDLUI_SetColor(SDLUI_Base.theme.col_border);
			r = {xx, yy + sa->h - sa->scrollbar_thickness, sa->track_length_h, sa->scrollbar_thickness};
			SDL_RenderFillRect(SDLUI_Base.renderer, &r);

			SDLUI_SetColor(SDLUI_Base.theme.col_thumb);
			sa->thumb_size_h = (float)(sa->track_length_h * sa->w) / (float)sa->content_width;
